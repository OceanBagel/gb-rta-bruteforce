package rta;

public class RedBlueAddr {
    
    // All ROM addresses used so far are identical for Red & Blue
    public static final int biosReadKeypadAddr = 0x021D;
    public static final int joypadAddr = 0x019A;
    public static final int postTIDAddr = 0x037860;
    public static final int animateNidorinoAddr = 0x105793;
    public static final int checkInterruptAddr = 0x12F8;
    public static final int joypadOverworldAddr = 0x0F4D;
    public static final int printLetterDelayAddr = 0x38D3;
    public static final int newBattleAddr = 0x0683;
    public static final int delayAtEndOfShootingStarAddr = 0x1058CB;
    public static final int textJingleCommandAddr = 0x1C31;
    public static final int enterMapAddr = 0x03A6;
    public static final int encounterTestAddr = 0x0478C4;
    public static final int igtInjectAddr = 0x1C766A;
    public static final int npcTimerExpireAddr = 0x0151FD;
    public static final int catchSuccessAddr = 0x035868;
    public static final int catchFailureAddr = 0x035922;
    public static final int manualTextScrollAddr = 0x3898;
    public static final int playCryAddr = 0x13D0;
    public static final int displayListMenuIdAddr = 0x2BE6;
    public static final int softResetAddr = 0x1F49;
    public static final int displayTitleScreenAddr = 0x0142DD;
    public static final int titleScreenPickNewMonAddr = 0x014496;
    public static final int initAddr = 0x1F54;
    public static final int displayTextBoxIdAddr = 0x30E8;
    public static final int displayNamingScreenAddr = 0x016596;
    public static final int calcStatAddr = 0x394A;
    public static final int updateNpcSpriteAddr = 0x014ED1;
}
